...
// import java.util.Stack; // Substituído por PilhaDePedidos
...
public class PedidoService {
    ...
    // Capacidade fixa para a pilha, conforme implementação didática
    ...
    // Substituído java.util.Stack pela implementação didática PilhaDePedidos
    private final PedidoStack filaPedidos = new PedidoStack(CAPACIDADE_FILA);
    ...
    public PedidoResponseDTO criarPedido(PedidoRequestDTO request) {
        ...
        // Adicionar pedido à fila (PilhaDePedidos)
        try {
            adicionarPedidoNaFila(pedidoResponse);
        } catch (RuntimeException e) {
            // A implementação PilhaDePedidos lança RuntimeException (ex: pilha cheia)
            log.error("Falha ao adicionar pedido ID {} na fila: {}", pedidoResponse.getId(), e.getMessage());
            // O pedido foi salvo, mas falhou ao entrar na pilha.
            // Dependendo da regra de negócio, pode ser necessário tratar este erro.
        }
    ...
    public List<PedidoResponseDTO> obterTodasAsMensagens() {
        ...
        // A ordem será do topo da pilha (último adicionado) para a base (primeiro
        // adicionado)

        // Modificado para usar o método da PilhaDePedidos,
        return filaPedidos.getTodosElementos();
    }
}